...
def getNaverSearch(node, srcText, start, display, config):
    ...
    responseDecode = reqUrl.getRequestUrl(
        url, config['CLIENT']['client_id'], config['CLIENT']['client_secret'])  # [CODE 1]
...
def init(data_path='.', save=True, config_path=None):
    ...
    for srcText in config['API']['search_words'].split():
        ...
        jsonResponse = getNaverSearch(
            node, srcText, 1, 100, config)  # [CODE 2]
        ...
        while ((jsonResponse != None) and (jsonResponse['display'] != 0)):
            ...
            jsonResponse = getNaverSearch(
                node, srcText, start, 100, config)  # [CODE 2]
        ...
        if save == True:
            ...
            csvDataFrame.to_csv(data_path + '/%s_naver_%s.csv' %
                                (srcText, node), encoding="utf-8-sig")
